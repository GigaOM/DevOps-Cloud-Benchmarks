...
resource "null_resource" "run_pre_reqs" {
  ...
  provisioner "file" {
    ...
    destination = "/root/bootstrap/vars.py"
  }

  provisioner "file" {
    content     = file("${path.module}/templates/pre_reqs.py")
    destination = "/root/bootstrap/pre_reqs.py"
  }

  provisioner "remote-exec" {
    inline = ["python3 /root/bootstrap/pre_reqs.py"]
  }
}
...
resource "null_resource" "copy_gcs_key" {
  ...
  provisioner "file" {
    ...
    destination = "/root/bootstrap/gcp_storage_reader.json"
  }
}
...
resource "null_resource" "download_vcenter_iso" {
  ...
  provisioner "file" {
    ...
    destination = "/root/bootstrap/download_vcenter.sh"
  }
  ...
  provisioner "remote-exec" {
    inline = ["bash /root/bootstrap/download_vcenter.sh"]
  }
}
...
resource "null_resource" "install_vpn_server" {
  ...
  provisioner "file" {
    ...
    destination = "/root/bootstrap/vpn_installer.sh"
  }

  provisioner "remote-exec" {
    inline = ["bash /root/bootstrap/vpn_installer.sh"]
  }
}
...
resource "null_resource" "copy_vcva_template" {
  ...
  provisioner "file" {
    ...
    destination = "/root/bootstrap/vcva_template.json"
  }
}
...
resource "null_resource" "copy_update_uplinks" {
  ...
  provisioner "file" {
    ...
    destination = "/root/bootstrap/update_uplinks.py"
  }
}
...
resource "null_resource" "esx_network_prereqs" {
  ...
  provisioner "file" {
    ...
    destination = "/root/bootstrap/esx_host_networking.py"
  }
}
...
resource "null_resource" "deploy_vcva" {
  ...
  provisioner "file" {
    ...
    destination = "/root/bootstrap/extend_datastore.sh"
  }

  provisioner "file" {
    content     = file("${path.module}/templates/vsan_claim.py")
    destination = "/root/bootstrap/vsan_claim.py"
  }

  provisioner "file" {
    content     = file("${path.module}/templates/deploy_vcva.py")
    destination = "/root/bootstrap/deploy_vcva.py"
  }

  provisioner "remote-exec" {
    inline = ["python3 /root/bootstrap/deploy_vcva.py"]
  }
}
...
resource "null_resource" "vsan_claim" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sleep 60",
      "python3 /root/bootstrap/vsan_claim.py"
    ]
  }
}
